...
let toDos = data.filter(item => item.completed !== true);
console.log(toDos);
// filter for user 6's tasks
let userSix = data.filter(item => item.userId === 6);
console.log(userSix);

//filter for user 6s completed tasks

let userSixCompletedTasks = userSix.filter(task => task.completed == true);
console.log(userSixCompletedTasks);
// filter for tasks with a title less than 24 characters long
let lessThanTwoFour = data.filter(item => item.title.length < 24);
console.log(lessThanTwoFour);